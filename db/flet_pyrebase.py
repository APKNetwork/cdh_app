...
class PyrebaseWrapper:
    ...
    def register_user(self, username, email, password):
        try:
            ...
            # Verificar el token y UUID
            if not self.idToken or not self.uuid:
                raise Exception("No se pudo obtener el token o UUID después del inicio de sesión.")
            
            # Actualizar la base de datos
            self.db.child("users").child(self.uuid).update(data={"name": "", "username": username}, token=self.idToken)
